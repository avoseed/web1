...
- `references/FRAMEWORKS.md` — **프레임워크 플레이북**. 논리 관계→프레임 선택표, 8개 원형(비교표·추이·매트릭스·피라미드·플로우·워터폴·카드·브릿지)의 언제/언제 아님, 반패턴 체크.
- `references/mockup_template.html` — **A단계 목업(승인용) HTML 스캐폴드**. 8개 프레임 원형을 톤·정렬 규정대로 담은 카드. 복제해 실제 장표처럼 채워 보여준다(빌드·렌더 없음).
...
1. **톤 선택 + 슬라이드별 프레임 판정** — 톤: 담백한 표준이면 `zetta_ppt_standard`, 네이비/빨강 임원 덱이면 `tone_v02`.
   **프레임은 슬라이드마다 논리 관계로 정한다**(`references/FRAMEWORKS.md` 선택표 참조): 대비→비교표/바차트, 시간→추이/타임라인 스파인, 위치→2×2 매트릭스, 구성→피라미드/트리, 흐름→프로세스/플로우, 인과→드라이버/워터폴, 병렬→분할 카드, 격차→브릿지.
   ⚠ **"리드+불릿+표+가로타임라인" 스택을 기본값으로 반복 금지** — 같은 프레임 3연속이면 재검토. 프레임을 **먼저** 고르고 내용을 채운다(내용 먼저 쓰고 표에 밀어넣기 금지).
2. **시각 목업 제시(기본) — HTML** : `references/mockup_template.html` 을 복제해 슬라이드마다 `.slide`
...
- **프레임 다양성**: 논리 관계가 프레임을 정한다(`references/FRAMEWORKS.md`). 한 스택(표+타임라인)의 내용만 갈아끼우는 반복 금지 — 슬라이드별로 매트릭스·피라미드·플로우·차트·카드·브릿지를 논리에 맞게 쓴다. **단, 프레임이 달라도 톤·색·정밀 기하(끝선 정렬·일정 간격·중앙선·70% 충전)는 그대로 지킨다.**
