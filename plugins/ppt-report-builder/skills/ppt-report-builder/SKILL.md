...
1. **톤 선택 + 슬라이드별 프레임 판정** — 톤: 담백한 표준이면 `zetta_ppt_standard`, 네이비/빨강 임원 덱이면 `tone_v02`.
   **프레임은 슬라이드마다 논리 관계로 정한다**(`references/FRAMEWORKS.md` 선택표 참조): 대비→비교표/바차트, 시간→추이/타임라인 스파인, 위치→2×2 매트릭스, 구성→피라미드/트리, 흐름→프로세스/플로우, 인과→드라이버/워터폴, 병렬→분할 카드, **전환(전/후)→As-Is/To-Be 대형 2박스+화살표**, 격차→브릿지.
