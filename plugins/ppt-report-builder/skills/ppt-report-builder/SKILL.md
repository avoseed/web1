...
**원페이저는 지배 프레임 1개(hero) + 정성 보조**(배경 불릿·실행 리본·각주)로 구성하고,
**한 페이지에 도식 여러 종류를 몽타주하지 않는다**(비인간적 — `FRAMEWORKS.md` §2-1). 논리가
지배 프레임 하나로 안 담기면 그건 원페이저가 아니라 **덱**이니 마디를 슬라이드로 펼친다.
...
- **원페이저 우선 · 지배 프레임 1개**: 기본 산출물은 원페이저 1장. **도식(hero) 1개 + 정성 보조**(배경 불릿·실행 리본·각주)로 구성하고 **한 장에 프레임 2종 초과 금지**(몽타주=비인간적, `FRAMEWORKS.md` §2-1). 여러 프레임이 필요하면 덱으로.
...
- **정보 밀도 · 공백 0(최우선)**: 우리 산출물의 최대 약점은 **정보량 부족.** 임원 보고서는 정보가 빽빽하다 — 각 요소를 구체 내용으로 **두껍게** 채운다. **화면에 `〔조사〕`/`__` 를 보이게 두지 않는다**(사람은 공백을 안 남긴다): 미확정 수치는 임의 생성 금지이되 그 자리를 **질적 서술(메커니즘·구조·인과·전략·운영)로 대체**하고, 실측 필요 수치는 각주 한 곳에 출처·갱신으로 모은다. **성의없는 축약 금지** — 로드맵은 한 줄 리본이 아니라 화살표+동그라미 타임라인으로 크게, 각 단계에 활동·목표 서술. 정량 스켈레톤 대신 정성 논리로 밀도. (SPEC §문구 2-1)
