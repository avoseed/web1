...
구성 원칙(SPEC 3-1): 논리 흐름에 따른 상하·좌우 구획 —
상단 2단(배경 | 개요) → 전폭 비교표 → 하단 2단(품목 비교 | 로드맵).
...
# ── 상단 2단: 도입 배경(Why) | 시범 운영 개요(What) ──
y = BODY_TOP_LEAD
section(s, MARGIN_L, y, HALF_W, "도입 배경")
bullet(s, MARGIN_L, y + GAP_HEAD, HALF_W, "이륜차 퀵커머스 소량 한정 → 대용량 공백")
bullet(s, MARGIN_L, y + GAP_HEAD + LINE_H, HALF_W, "바로퀵 매출 197%↑(6월) → 수요 실증")

section(s, COL2_L, y, HALF_W, "시범 운영 개요 (7. 9. 개시)")
bullet(s, COL2_L, y + GAP_HEAD, HALF_W, "20시 주문 마감 → 2시간 내 배송")
bullet(s, COL2_L, y + GAP_HEAD + LINE_H, HALF_W, "사륜차 대용량·중량 상품 즉시 배송")
