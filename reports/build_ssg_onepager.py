...
def lead(slide, t, text):
    """리드메시지: 제목 하단 볼드 검정 한 줄."""
    return _txt(slide, MARGIN_L, t, 25.26, 0.7, text, size=14, bold=True, color=BLACK)


def section(slide, l, t, w, text):
    """검정 볼드 소제목 (□ 섹션 라벨)."""
    return _txt(slide, l, t, w, 0.55, "□ " + text, size=13, bold=True, color=BLACK)
...
# 리드메시지 (y 3.0)
lead(s, 3.00, "'26. 7. 9. 이마트 양재·하남점 '2시간 내 배송' 시범 도입 — 대용량 장보기 수요 공략, PP센터 가동률 제고")

# 1. 도입 배경 (y 4.0~5.7)
section(s, MARGIN_L, 4.00, 25.26, "도입 배경")
bullet(s, MARGIN_L, 4.55, 25.26,
       "이륜차 퀵커머스(바로퀵)는 적재량 한계로 소량 배송에 한정 → 대형마트 주력인 3~4인 가구 대용량 장보기 공백")
bullet(s, MARGIN_L, 5.05, 25.26,
       "바로퀵 6월 매출 1월 대비 197% 증가로 온디맨드 수요 실증 → 1시간(소량·이륜차) / 2시간(대용량·사륜차) 세분화 대응 (B마트도 대용량 확대 중)")

# 2. 시범 운영 개요 (y 5.85~7.55)
section(s, MARGIN_L, 5.85, 25.26, "시범 운영 개요")
bullet(s, MARGIN_L, 6.40, 25.26,
       "오후 8시까지 주문 시 결제 시점 기준 2시간 내 배송 완료, 기존 예약배송 병행 선택 가능")
bullet(s, MARGIN_L, 6.90, 25.26,
       "쓱배송 사륜 차량 활용으로 무거운 대용량 상품까지 즉시 배송 — 점포당 약 15만 종 상품이 기반")

# 3. 배송 서비스 비교 (y 7.75~13.1)
section(s, MARGIN_L, 7.75, 25.26, "이마트·SSG닷컴 배송 서비스 비교")
data = [
...
tbl = add_fin_table(s, MARGIN_L, 8.30, 25.26, 4.8, data,
                    col_w=[3.9, 5.3, 2.5, 3.6, 9.96], header_rows=1, font_size=10)
left_align_body(tbl, [1, 3, 4])

# 4-좌. 취급 품목 수 비교 (y 13.35~16.85)
section(s, MARGIN_L, 13.35, HALF_W, "취급 품목 수 비교")
data = [
    ["구분", "품목 수", "비고"],
...
tbl = add_fin_table(s, MARGIN_L, 13.90, HALF_W, 3.0, data,
                    col_w=[3.5, 3.6, 5.2], header_rows=1,
                    header_fill=TH_SECOND, font_size=10)
left_align_body(tbl, [2])

# 4-우. 확대 로드맵 (y 13.35~16.85)
section(s, COL2_L, 13.35, HALF_W, "확대 로드맵")
data = [
    ["시기", "계획"],
...
tbl = add_fin_table(s, COL2_L, 13.90, HALF_W, 3.0, data,
                    col_w=[3.9, 8.4], header_rows=1,
                    header_fill=TH_SECOND, font_size=10)
left_align_body(tbl, [1])

# 규제 참고 (y 17.1)
_txt(s, MARGIN_L, 17.10, 25.26, 0.5,
