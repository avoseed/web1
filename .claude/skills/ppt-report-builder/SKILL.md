...
**기본 산출물은 원페이저(A4 가로 1장)** — 요청이 덱을 명시하지 않으면 원페이저. 프레임 이전에
**문서 유형**부터 판별한다(`FRAMEWORKS.md` §2-1): **설득·제안형**(하나의 So-What 관철)이면 **지배
프레임 1개(hero) + 정성 보조**; **현황·대시보드형**(보유·상태+관리체계 공유)이면 **영역 구획(좌우
2단 등) + 영역별 프레임 1개씩**(표+리스트+플로우 공존 정상). 여러 도식을 **구획 없이 세로로 쌓는 몽타주는
금지** — 나누려면 영역·소제목으로 구획한다. 지배 프레임 하나로 안 담기는 설득 논리면 → **덱**.

빌드(python-pptx)와 렌더 QA(LibreOffice→PNG, 이상 시 재렌더 반복)는 **토큰 비용이 크다.**
그래서 **A단계에서 저비용 목업으로 구조·내용을 먼저 확정**하고, **승인 뒤에만** B단계
...
3. **시각 목업 제시(기본) — HTML** : `references/mockup_template.html` 을 복제해 슬라이드마다 `.slide`
   ...
   - 규칙은 목업에서부터: 결론=리드, 파란 글자 금지, 부호 앞 공백 금지, 순한글, 정보 밀도 높게(공백 노출 금지).
   - **여백·충전(1.5cm 액자 — SPEC §3-4-0, 최우선)**: 좌·우·하단 **1.5cm 여백**(HTML 목업은 960×664 기준 ≈**52px** L/R/하단 패딩)을 남기고, 그 안쪽은 **빈 곳 없이 충전.** 세로로 요소를 분배해 **마지막 객체(각주)가 하단 여백선에 닿게**(밑 휑함 금지), 가로도 폭 끝선까지. 목업에서 바닥까지 채워 확인한다.
...
- **문서 유형 → 작법**(`FRAMEWORKS.md` §2-1): **설득·제안형**=지배 프레임 1개+정성 보조 / **현황·대시보드형**=영역 구획(좌우 2단 등)+영역별 프레임 1개씩. **몽타주(구획 없이 도식 쌓기) 금지**, 여러 프레임은 영역·소제목으로 **구획**해 담는다.
- **논리 골격 먼저**: 프레임 선택 이전에 **스토리라인**(SCQA/Pyramid/문제-원인-해법/현황-목표-격차-실행)을 세운다(`FRAMEWORKS.md` §1). 덱은 마디를 슬라이드로, 원페이저는 리드+배경 불릿+hero+실행 리본으로 압축.
...
- **완성도(최우선) = 정보 밀도 + 하단 채움 + 지표 공란 허용**: 최대 약점은 **정보량 부족·하단 여백**(공란 자체는 문제 아님, SPEC §문구 2-1). ① **정보 밀도** — 각 요소를 구체 내용으로 두껍게, "임원이 이 한 장에서 얼마나 알게 되나" 자문. **성의없는 축약 금지**(로드맵은 화살표+동그라미 타임라인으로 크게). ② **하단 여백 방치 금지(재발 1순위)** — 요소를 세로로 분배해 페이지 하단까지, 각주 마지막 객체 직하(목업부터 확인). ③ **데이터 정직성** — 모르는 수치는 임의 생성 금지, 특정 지표 자리는 `〔조사〕`/`__`/`0` 표기 **좋음**(완성도 신호). 단 지표 자리표가 화면 대부분인 정량 스켈레톤은 금지 → 질적 서술로 밀도.
- **위계형 불릿 선호**: 평면 나열 대신 **`•` 포괄 문구(요지) → `-` 세부 보완(근거·수치·예시)** 2단 위계(`add_bullets` level 0/1). 시각 정돈·빠른 파악(SPEC §2-2).
