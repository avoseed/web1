...
| `add_vtimeline(slide, l, t, w, h, steps, max_gap)` | 세로 타임라인 — 좁은 컬럼(6:4 우측) 세로 채움. `max_gap` 노드 간격 상한 |
...
### A. 6:4 본장형 (경쟁사 동향·현황→전개) — 기본값
좌 60% 현황·구조(불릿 + 담백한 표), 우 40% 전개(불릿 + 세로 타임라인 + 결론 박스).
컬럼 헤더 + 중앙 세로선. **좌 표 하단 = 우 결론 박스 하단 동일선.** → `reports/_template_onepager.py`

### B. 상하 밴드 적층 (전폭 밴드 + 1개 밴드만 좌우 분할)
전폭 밴드를 위→아래 적층, 한 밴드만 좌우 분할. 로드맵은 `add_htimeline`(가로).
전체 좌우 2단이 아니므로 바닥선 정렬 부담이 적다.

### C. 재무 전폭 표 / 근거 표 + 해설
상단 전폭 `add_fin_table` → 하단 2~3단 분해(【 】 + 불릿). 손익·KPI·비교.

프레임워크 선택은 **논리 흐름**이 정한다(문제→해법=좌우 2단, 근거→실행=상하). 단순 상하 나열 금지.
...
1. **끝선 정렬**: 한 컬럼의 전 객체(헤더 바·불릿·표)는 **동일 폭**으로 좌·우 끝선을 맞춘다.
   표 `w` = 컬럼 헤더 `w` = `LW`. 서로 다른 폭 금지.
2. **일정 간격**: 블록 간 간격을 토큰 하나(`GB`)로 통일. 들쭉날쭉한 여백 금지.
3. **중앙 세로선 침범 금지**: 표 끝선과 중앙 세로선 사이 **여백(≥0.5cm)** 확보,
   좌·우 대칭. 컬럼 간격 `GAP` 을 넓혀(0.66→1.10) 확보한다. 표가 세로선에 **닿으면 안 됨**.
4. **표 밀도**: 행을 늘어뜨리지 말 것. 표는 **작고 조밀하게**, 필요 시 상단 불릿으로
   채우고 표를 그 아래 배치.
5. **하단 처리 — 2가지 모드 중 택1** (하단 여백은 재발 금지 1순위):
   - **채움 모드**: 밀도를 다소 넓히더라도 객체를 하단(≈17cm)까지 분산해 페이지를 채우고
...
6. **좌우 바닥선 정렬**: 좌우 2단은 두 컬럼의 최하단 객체 하단선을 **같은 y**로.

측정 검증(빌드 후): 표 우측 끝선과 `VSEP_X` 간 거리, 좌우 컬럼 하단 y, 각주 y 를 실측해 확인.
...
레이아웃: `BODY_W=25.16` `MARGIN_L=MARGIN_R=1.18` `COL_GAP=0.66`(2단은 1.10 권장).
